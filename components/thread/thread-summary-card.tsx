...
export function ThreadSummaryCard({ threadId, initialSummary, className }: ThreadSummaryCardProps) {
  ...
  const requestSummary = useCallback(async function () {
    ...
    try {
      ...
      // The summary is generated by a background job and persisted on the thread.
      // There is no job-status endpoint, so tell the user to come back for it.
      toasts.info('Summary is generating. It will appear here shortly.');
      router.refresh();
    } catch (error) {
      if (!mountedRef.current) return;
