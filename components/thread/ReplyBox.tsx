...
export default function ReplyBox({
  ...
  const applyInlineFormat = useCallback(
    (action: ToolbarAction) => {
      const textarea = document.getElementById('thread-reply-box') as HTMLTextAreaElement | null;
      if (!textarea) return;
      ...
      const next = value.slice(0, start) + wrapped + value.slice(end, value.length);
      setValue(next);
