...
import { Hash, Sparkles, Users, MessageSquare, ShieldCheck, TrendingUp } from "lucide-react";
...
export default async function ThreadPage({ params }: { params: { slug: string } }) {
  ...
  const allMessages: Message[] = thread.messages.map((m: any) => ({
    id: m.id,
    content: m.content,
    ...
    sender: { 
      name: m.sender?.name || m.senderName || "Anonymous", 
      image: m.sender?.image || m.senderAvatar || null 
    },
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest">Active Discussion</span>
              </div>
            </div>
          </div>
          ...
          <div className="max-w-4xl mx-auto p-6">
            {allMessages.length === 0 ? (
              <div className="text-center py-12">
                <p className="text-zinc-400 mb-4">No comments yet. Be the first to comment!</p>
              </div>
            ) : (
              <CommentTree 
      ...
      <aside className="w-[340px] flex flex-col bg-[#161618] overflow-y-auto">
        <div className="p-8 border-b border-zinc-800/50">
          <div className="flex items-center gap-2 mb-6">
            <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Thread Details</p>
          </div>
          ...
          <div className="space-y-4 bg-[#1C1C1E] p-4 rounded-xl border border-zinc-800/50">
            <StatLine icon={<Users size={14} />} label="Active Members" value={thread.activeUsers} />
            <StatLine icon={<MessageSquare size={14} />} label="Total Messages" value={thread.messageCount} />
          </div>
        ...
        <div className="p-6">
          <div className="rounded-2xl bg-linear-to-br from-[#1C1C1E] to-[#161618] border border-zinc-800 p-6 relative overflow-hidden group">
            <div className="flex items-center gap-2 mb-3 text-white">
              ...
              <span className="text-xs font-bold uppercase tracking-widest">AI Synthesis</span>
            </div>
            ...
            <div className="relative z-10">
              {thread.summary ? (
                ...
                <div className="flex flex-col gap-2">
                   ...
                   <p className="text-[10px] text-zinc-600 mt-2 italic">Analyzing recent logs...</p>
                </div>
              )}
            </div>
        ...
        {isAdmin(session.user) && (
          <div className="p-6 mt-auto">
            <div className="rounded-xl border border-dashed border-zinc-800 p-5">
              ...
              <Link href="/dashboard/admin" className="block w-full text-center py-2.5 bg-zinc-800 hover:bg-zinc-700 text-white text-xs font-bold rounded-lg transition-colors">
                Manage Lifecycle
              </Link>
            </div>
          </div>
