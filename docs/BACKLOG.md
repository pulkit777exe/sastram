...
| O1b | **No CSRF token validation on server actions** (ARCHITECTURE-REPORT Critical Issue #3). Currently accepted as "acceptable for current scope," resting entirely on Next.js SameSite cookie behavior. Real gap, not a non-issue: low risk *today* because `frame-ancestors 'none'` is enforced and there is no CORS-exposed API surface (no third-party embeds / cross-origin form posts). Risk rises if either changes (e.g., CORS opened, iframe embedding added). | Architecture report Critical Issue #3. | **Recorded so "acceptable for current scope" doesn't silently become permanent.** Re-evaluate if the attack surface changes (CORS exposure, embeds). If hardened later, add a double-submit/header token check on mutating server actions. |
