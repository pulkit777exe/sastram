...
> **Update 2026-09-08:** Re-audited. `C1` dispatcher now exists (`modules/notifications/dispatcher.ts` via `lib/queue/workers/ai-jobs: dispatch`), `C7` `createTopic` now delegates to `threads-write/createThread` (not duplicate `prisma.create`), `C3` no `newsletter/service.ts` file on disk (already removed), `C2` still split (invitations + members), `C4-C6,C8` still open. Counts drift fixed: 29 modules / 35 models / 46 routes / 53 tests (was 24/30/35/46). New moat: `Bounty`, `Poll.isMarket`/`resolvedOptionIndex`/`marketResolvedAt` markets, `Collection`+`CollectionItem` + `KnowledgePage` (resolutionScore>85+verifiedAt), `Thread.forkedFromId` + fork APIs (`/api/threads/[threadId]/fork`, `/api/threads/fork-external`), `Message.factCheckStatus` (`UNCHECKED`/`VERIFIED`/`DISPUTED`), `AppealVote` jury (3 random `MODERATOR`, 2/3 majority), Worker sandbox `code-runner`, `force-graph` canvas (`components/graph/ForceGraphCanvas.tsx` dynamic `import('force-graph')`, 360–560px, drag/zoom, `ThreadRelation` similarity ≥0.7), Deep Research advanced 12-source (`GENERATE_DEEP_RESEARCH`, `exaMode: agentic` + `tavilyMode: research`, Exa `numResults: 12`/`maxCharacters: 12000`, Tavily `max_results: 12`/`search_depth: advanced`), Canvas `crossReference` tier sort + conflict detection (`/dashboard/canvas?left=&right=`), verified resolution decay (`Thread.verifiedAt/By` + `lastVerifiedAt`, `modules/threads/confidence-decay.ts` 90d half-life, `computeConfidence` <0.5 → `isOutdated`), weekly `refreshUserExpertise` reuse in `update-threads` cron (`lib/services/user-memory.ts`, 50 active users, `threadDna.expertiseLevel` + `AiSearchSession.queryType` → `preferences.expertiseLevel` injected in `synthesis`).
